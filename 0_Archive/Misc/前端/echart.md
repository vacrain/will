...
## 第一种，引入echarts文件，使用步骤：

1. 页面中引入echarts.js，其他方法详见【引入js方法】

2. - <script src="https://cdn.staticfile.org/echarts/4.3.0/echarts.min.js"></script>

3. 准备dom，id为main的div

4. 基于dom，初始化echarts对象

5. - var myChart = echarts.init(document.getElementById('main')

6. 将实例中调好配置的图表配置赋值给变量option

7. 根据配置显示图标，myChart.setOption(option)
...
2. - 这一步会将echarts依赖写入package.json中
   - 并在node_modules文件夹中多出两个文件夹：echarts和zrender（**Canvas类库）**

3. 引入echarts：var echarts = require('echarts');
...
4. 定制js：https://echarts.apache.org/zh/builder.html
